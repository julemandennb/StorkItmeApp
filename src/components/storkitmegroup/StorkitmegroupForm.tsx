...
export type StorkitmegroupData = {
  name: string;
  description: string;
  users: users[];
...
type Props = {
  initialValues?: StorkitmegroupData;
  usersList: users[];
  ...
  onSubmit: (data: StorkitmegroupData) => boolean;
  onDelete?: () => boolean;
...
const emptyValues: StorkitmegroupData = {
  name:'',
  description:'',
  users:[],
  storkItmes:[],
...
export function StorkitmegroupForm({
  initialValues,
  onSubmit,
  onDelete,
  onRemoveUser,
...
const [form,setForm] = useState<StorkitmegroupData>(
...
function update<K extends keyof StorkitmegroupData>(
 key:K,
 value:StorkitmegroupData[K]
){
 setForm(prev=>({
   ...prev,
   [key]:value
